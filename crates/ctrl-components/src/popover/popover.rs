...
pub fn Popover(props: PopoverProps) -> Element {
    ...
    // ── 窗口 resize 时重新计算位置 ──
    {
        ...
        let card_id = card_id.clone();
        let placement = props.placement.clone();
        let visible_for_resize = visible.clone();
        use_effect(move || {
            let trigger_id = trigger_id.clone();
            let card_id = card_id.clone();
            let placement = placement.clone();
            ...
            let callback = Closure::wrap(Box::new(move || {
                if visible_for_resize.try_read().is_ok_and(|r| *r) {
                    position_card(&trigger_id, &card_id, &placement);
                }
            }) as Box<dyn FnMut()>);
            ...
            callback.forget(); // 保持 callback 存活，随组件生命周期
        });
    }
    ...
    // ── 点击外部关闭 ──
    {
        let trigger_id = trigger_id.clone();
        let card_id = card_id.clone();
        let mut visible_close = visible.clone();
        use_effect(use_reactive((&visible,), move |(visible,)| {
            if !visible() {
                return;
            }
            ...
            let callback = Closure::wrap(Box::new(move |event: web_sys::MouseEvent| {
                ...
                if let (Some(trigger), Some(card)) = (
                    ...
                    if !trigger.contains(Some(el)) && !card.contains(Some(el)) {
                        if let Ok(mut w) = visible_close.try_write() { *w = false; }
                    }
                }
            }) as Box<dyn FnMut(_)>);
    ...
    // ── 滚动时关闭 ──
    {
        let mut visible_scroll = visible.clone();
        use_effect(use_reactive((&visible,), move |(visible,)| {
            if !visible() {
                return;
            }
            let callback = Closure::wrap(Box::new(move || {
                if let Ok(mut w) = visible_scroll.try_write() { *w = false; }
            }) as Box<dyn FnMut()>);
            ...
            if let Some(window) = web_sys::window() {
                let _ = window.add_event_listener_with_callback_and_bool(
                    ...
                    true, // capture: true 捕获所有滚动事件（包括祖先容器滚动）
                );
            }
